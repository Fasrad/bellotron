...
sub bellofy{    # bellows-drawing algorithm.
    ...
    foreach $pleat (0..$numpleats){
        ...
        $ytmp=$pleat*$meanpleat; 

        $xtmp=-$riw/2; # add trig to move right xtmp 

        $x2tmp=$riw/2; # add trig to move left x2tmp
    ...
    foreach $pleat (0..$numpleats-1){
        $xtmp=-$row/2;
        $ytmp=$pleat*$meanpleat+$meanpleat/2; 
        $x2tmp=+$row/2;
        print $ps "%outer (longer) line for pleat $pleat of panel I\n";
        &line($xtmp, $ytmp, $x2tmp, $ytmp);
    }
    # draw left zigzags A
    foreach $pleat (0..$numpleats-1){
        ...
        $ytmp=$pleat*$meanpleat; 
        $x2tmp=-$row/2;
        $y2tmp=$pleat*$meanpleat+$meanpleat/2; 
        print $ps "%zig\n";
    ...
    foreach $pleat (0..$numpleats-1){
        $xtmp=-$riw/2;
        $ytmp=($pleat+1)*$meanpleat; 
        $x2tmp=-$row/2;
        $y2tmp=($pleat+1)*$meanpleat-$meanpleat/2; 
        print $ps "%zag\n";
        &line($xtmp, $ytmp, $x2tmp, $y2tmp);
    }
...
sub get_UI{
   print "enter your rear standard inner bellows width in mm ";
    ...
    Rear outer width=$row mm,
    Rear inner width=$riw mm, 
    Front outer width=$fow mm,
    Front inner width=$fiw mm,
    Extended length = $length mm...\n\n";

    # convert units to "ps points" for internal use
    for ($row, $riw, $roh, $rih, $fow, $foh, $row, $riw){
        $_/=72;   # 72 points per inch; 
        $_*=25.4; # 25.4 mm per inch;
    }
